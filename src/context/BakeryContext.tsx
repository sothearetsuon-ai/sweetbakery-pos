...
export const BakeryProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const applyLanData = (data: any) => {
    ...
    if (Array.isArray(data.expenses)) {
      const activeExpenses = data.expenses.filter((e: any) => e && e.id && !deletedExpenseIds.current.has(e.id));
      setExpenses(activeExpenses);
      try { localStorage.setItem('bakery_expenses', JSON.stringify(activeExpenses)); } catch (e) {}
    }
    if (data.storeInfo) {
      setStoreInfo((prev) => {
  ...
  useEffect(() => {
    ...
    const unsubExpenses = subscribeToFirestoreCollection<Expense>('expenses', (cloudExpenses) => {
      if (cloudExpenses && cloudExpenses.length > 0) {
        setExpenses(cloudExpenses.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()));
      }
    });

...
export const CartPanel: React.FC<CartPanelProps> = ({ onCheckout, onClose, isMobileSheet = false }) => {
  ...
  return (
    ...
    >
      ...
      <div className="px-4 pt-2.5 pb-1 bg-white/90 border-b border-rose-100/50">
        {!showCustomPriceForm ? (
          <button
            type="button"
            onClick={() => {
            ...
            <div className="flex items-center gap-2 pt-0.5">
              <div className="flex items-center bg-white border border-pink-200 rounded-xl p-0.5">
                <button
                  ...
                  className="w-6 h-6 rounded-lg bg-pink-50 hover:bg-pink-100 text-pink-700 flex items-center justify-center cursor-pointer"
                >
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3">
        {cart.length === 0 ? (
          <div className="h-full flex flex-col items-center justify-center text-slate-400 py-12 text-center select-none">
            <div className="w-20 h-20 rounded-3xl bg-pink-50/70 border border-pink-100/80 flex items-center justify-center mb-3 text-pink-400">
              <ShoppingBag className="w-9 h-9 animate-bounce" />
            </div>
      ...
      {cart.length > 0 && (
        <div className="p-4 bg-white/90 border-t border-rose-100/80 space-y-3 shadow-lg">
          {/* Promo code field */}

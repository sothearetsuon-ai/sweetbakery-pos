...
export const getStoreId = (): string => {
  ...
  // If id is empty or was auto-generated by the previous migration bug (STORE-XXXX-XXXX), reset to default
  if (!id || /^STORE-[A-F0-9]{4}-[A-F0-9]{4}$/i.test(id)) {
    id = DEFAULT_STORE_ID;
...
export const getScopedCollectionRef = (db: Firestore, collectionName: string) => {
  ...
  if (!storeId || storeId === 'default' || storeId === 'root' || storeId === 'STORE-DEFAULT') {

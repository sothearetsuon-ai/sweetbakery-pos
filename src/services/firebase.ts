...
/**
 * Check if a Store Tenant ID is considered the default primary store (ហាងវិជ្ជតា)
 */
export const isDefaultStore = (id?: string | null): boolean => {
  if (!id) return true;
  const s = id.trim().toLowerCase();
  return s === 'default' || s === 'root' || s === 'store-default' || s === '';
};

/**
 * Get or initialize persistent Store Tenant ID
 * 'DEFAULT' maps to root collections (preserving legacy and primary store data for ហាងវិជ្ជតា).
 * Any custom ID (e.g. 'STORE-SHOP2') maps to tenants/{storeId}/*.
 */
export const getStoreId = (): string => {
  let id = localStorage.getItem(STORAGE_STORE_ID);

  // If id is empty or was auto-generated by the previous migration bug (STORE-XXXX-XXXX), or is default, normalize to DEFAULT
  if (!id || /^STORE-[A-F0-9]{4}-[A-F0-9]{4}$/i.test(id) || isDefaultStore(id)) {
    id = DEFAULT_STORE_ID;
...
/**
 * Set custom Store Tenant ID (e.g. to isolate data for another shop or connect devices)
 */
export const setStoreId = (newStoreId: string): void => {
  const cleanId = newStoreId.trim().toUpperCase().replace(/[^A-Z0-9_-]/g, '') || DEFAULT_STORE_ID;
  const finalId = isDefaultStore(cleanId) ? DEFAULT_STORE_ID : cleanId;
  localStorage.setItem(STORAGE_STORE_ID, finalId);
  idbSet(STORAGE_STORE_ID, finalId).catch(() => {});
  window.dispatchEvent(new CustomEvent(STORE_ID_EVENT, { detail: { storeId: finalId } }));
};
